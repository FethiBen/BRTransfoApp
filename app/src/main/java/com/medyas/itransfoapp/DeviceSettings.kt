...
class DeviceSettings : AppCompatActivity() {
    ...
    inner class getParamTask(obj: String) : AsyncTask<Void, Void, Result<Any, FuelError>>() {
        ...
        override fun doInBackground(vararg params: Void?): Result<Any, FuelError> {
            val (request, response, result) = "https://fethiben.github.io/RB-TransfosWeb/getparameters/".httpPost().body(p).responseString()
            return result
        }
    ...
    inner class setParamTask(obj: String) : AsyncTask<Void, Void, Result<Any, FuelError>>() {
        private var p = obj
        override fun doInBackground(vararg params: Void?): Result<Any, FuelError> {
            val (request, response, result) = "https://fethiben.github.io/RB-TransfosWeb/updateprameters/".httpPost().body(p).responseString()
            return result
        }
